...
class WeexTemplateCommand(sublime_plugin.TextCommand):

    def run(self, edit, type="vue", paths = [None]):
        print('WeexTemplate')
        view = self.view
        ...
        self.tab = self.creat_tab(view, type,paths)
    ...
    def creat_tab(self, view, type,paths = [None]):
        win = view.window()
...
class WeexTemplateReplaceCommand(sublime_plugin.TextCommand):
...
class WeexTemplateEventListener(sublime_plugin.EventListener):
    def __init__(self):
        self.unsaved_ids = {}
...
def plugin_loaded():  # for ST3 >= 3016
    ...
    if not os.path.isdir(TARGET_PATH):
        ...
        file_list = [
            ...
            'WeexTemplate.py',
            'README.md',
            tmpl_dir + 'weex.tmpl', tmpl_dir + 'vue.tmpl'
        ]
        try:
